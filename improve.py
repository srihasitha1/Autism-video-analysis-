...
# CONSTANTS
# ─────────────────────────────────────────────────────────────────────────────
MODEL_P2 = "autism_mobilenet_gru_phase2.h5"
MODEL_P1 = "autism_mobilenet_gru_phase1.h5"
RESULTS  = []
...
def improvement_2(X_train, y_train, X_val, y_val, X_test, y_test, config, baseline_acc):
    ...
    model.save("autism_improved_mixup.h5")
...
def improvement_3(X_train, y_train, X_val, y_val, X_test, y_test, config, baseline_acc):
    ...
    model.save("autism_improved_label_smooth.h5")
...
def improvement_4(X_train, y_train, X_val, y_val, X_test, y_test, config, baseline_acc):
    ...
    model.save("autism_improved_disc_lr.h5")
...
def improvement_5(X_train, y_train, X_val, y_val, X_test, y_test, config, baseline_acc):
    ...
    model.save("autism_improved_cosine.h5")
    best_val = max(h.history["val_accuracy"])
...
def improvement_6(X_train, y_train, X_val, y_val, X_test, y_test, config, baseline_acc):
    ...
    candidates = ["autism_improved_disc_lr.h5", "autism_improved_cosine.h5",
                   "autism_improved_label_smooth.h5", "autism_improved_mixup.h5"]
    for c in candidates:
        if os.path.exists(c):
            best_path = c
            break
    ...
    model.save("autism_mobilenet_swa.h5")
...
def improvement_7(X_train, y_train, X_val, y_val, X_test, y_test, config, baseline_acc):
    ...
    # Compute optical flow for all splits
    def compute_flow(X, label=""):
        flows = []
        for i in range(len(X)):
            flows.append(extract_flow_clip(X[i].astype(np.float32), flow_h, flow_w))
            if (i + 1) % 50 == 0:
                print(f"    {label} flow: {i+1}/{len(X)}")
        return np.array(flows, dtype=np.float16)
    ...
    dual_model.save("autism_improved_dual_stream.h5")
    best_val = max(h.history["val_accuracy"])
...
def main():
    ...
    best_files = {
        ...
        2: "autism_improved_mixup.h5",
        3: "autism_improved_label_smooth.h5",
        4: "autism_improved_disc_lr.h5",
        5: "autism_improved_cosine.h5",
        6: "autism_mobilenet_swa.h5",
        7: "autism_improved_dual_stream.h5",
    }
    ...
    if os.path.exists(best_file):
        ...
        best_model.save("autism_final.h5")
        print(f"  Best model saved → autism_final.h5")
