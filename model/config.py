...
CONFIG = {
    ...
    # ── Paths ──────────────────────────────────────────────────────────────────
    # All save paths are RELATIVE to output_dir. Change output_dir to relocate
    # every generated file at once (e.g. to a Google Drive folder in Colab).
    "output_dir":               ".",
    ...
    # Phase checkpoints & final model
    "phase1_save_name":         "autism_mobilenet_gru_phase1.h5",
    "phase2_save_name":         "autism_mobilenet_gru_phase2.h5",
    "final_save_name":          "autism_final.h5",
    "swa_save_name":            "autism_mobilenet_swa.h5",
    "improved_save_prefix":     "autism_improved",
...
def output_path(config: dict, filename: str) -> str:
    """
    Resolve a filename relative to config["output_dir"].

    Usage:
        from config import CONFIG, output_path
        path = output_path(CONFIG, CONFIG["phase1_save_name"])
        model.save(path)

    On a local machine with output_dir=".", this returns "autism_...h5".
    In Colab with output_dir="/content/drive/MyDrive/autism_outputs/",
    it returns the full Drive path — making the pipeline fully relocatable.
    """
